...
Deno.serve(async (req) => {
  ...
  // (a flaky Supabase read, a network error on the completion call --
  // not just an error inside the Gemini tool-call loop, which already
  // had its own narrower try/catch below) reliably releases the claim
  // immediately, instead of leaving a campaign's Continue button stuck
  // until the 5-minute lease in claim_ai_gm_turn times out on its own.
  try {
